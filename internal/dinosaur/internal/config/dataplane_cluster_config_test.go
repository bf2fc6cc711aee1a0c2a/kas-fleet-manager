...
func TestDataplaneClusterConfig_IsWithinClusterLimit(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{
				DataPlaneClusterScalingType: ManualScaling,
				ClusterList: ClusterList{
					ManualCluster{ClusterId: "test01", DinosaurInstanceLimit: 3},
				},
			},
			args: args{
				clusterId: "test01",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := conf.IsNumberOfDinosaurWithinClusterLimit(tt.args.clusterId, tt.args.count); got != tt.want {
