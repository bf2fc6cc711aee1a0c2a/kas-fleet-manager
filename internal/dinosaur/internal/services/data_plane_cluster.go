...
// the data plane cluster based on the reported status by the fleet shard
...
func (d *dataPlaneClusterService) updateDataPlaneClusterNodes(cluster *api.Cluster, status *dbapi.DataPlaneClusterStatus) (int, error) {
	// Current assumptions of how the fleet-shard-operator works:
...
func (d *dataPlaneClusterService) setClusterStatus(cluster *api.Cluster, status *dbapi.DataPlaneClusterStatus) error {
	...
	if d.DataplaneClusterConfig.IsDataPlaneAutoScalingEnabled() {
		var err error
		remainingCapacity, err = d.dinosaurClustersCapacityAvailable(status, d.minimumDinosaurCapacity())
		if err != nil {
			return err
		}
	}
...
// reported remaining dinosaur attributes is smaller than its corresponding
...
// dinosaur attributes is higher than its corresponding scale-down capacity threshold
...
// dinosaurClustersCapacityAvailable returns true when all the reported remaining
// dinosaur attributes are higher than their corresponding capacity attribute
// provided in minCapacity
func (d *dataPlaneClusterService) dinosaurClustersCapacityAvailable(status *dbapi.DataPlaneClusterStatus, minCapacity *dataPlaneComputeNodesDinosaurCapacityAttributes) (bool, error) {
...
func (d *dataPlaneClusterService) scaleDownThresholds(status *dbapi.DataPlaneClusterStatus) *dataPlaneComputeNodesDinosaurCapacityAttributes {
	var res *dataPlaneComputeNodesDinosaurCapacityAttributes = &dataPlaneComputeNodesDinosaurCapacityAttributes{}
...
func (d *dataPlaneClusterService) scaleUpThresholds(status *dbapi.DataPlaneClusterStatus) *dataPlaneComputeNodesDinosaurCapacityAttributes {
	return d.minimumDinosaurCapacity()
}

// minimumDinosaurCapacity returns the minimum Dinosaur Capacity attributes needed
// to consider that a dinosaur cluster has capacity available
func (d *dataPlaneClusterService) minimumDinosaurCapacity() *dataPlaneComputeNodesDinosaurCapacityAttributes {
	return &dataPlaneComputeNodesDinosaurCapacityAttributes{
		Connections: d.DinosaurConfig.DinosaurCapacity.TotalMaxConnections,
		Partitions:  d.DinosaurConfig.DinosaurCapacity.MaxPartitions,
	}
}
