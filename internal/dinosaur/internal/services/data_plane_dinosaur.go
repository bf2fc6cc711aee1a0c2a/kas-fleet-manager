...
func (d *dataPlaneDinosaurService) UpdateDataPlaneDinosaurService(ctx context.Context, clusterId string, status []*dbapi.DataPlaneDinosaurStatus) *serviceError.ServiceError {
	...
	for _, ks := range status {
		...
		case statusReady:
			// for now, only store the routes (and create them) when the Dinosaurs are ready, as by the time they are ready,
...
func (d *dataPlaneDinosaurService) setDinosaurClusterReady(dinosaur *dbapi.DinosaurRequest) *serviceError.ServiceError {
	...
	if ok, err := d.dinosaurService.UpdateStatus(dinosaur.ID, constants2.DinosaurRequestStatusReady); ok {
		if err != nil {
...
func (d *dataPlaneDinosaurService) setDinosaurRequestVersionFields(dinosaur *dbapi.DinosaurRequest, status *dbapi.DataPlaneDinosaurStatus) *serviceError.ServiceError {
	...
	// TODO for now we don't set the dinosaur_upgrading attribute at all because
	// kas fleet shard operator still does not explicitely report whether dinosaur
	...
	// dinosaur.DinosaurUpgrading = dinosaurBeingUpgraded
	// prevDinosaurUpgrading := dinosaur.DinosaurUpgrading
	// dinosaurBeingUpgraded := dinosaur.DesiredDinosaurVersion != dinosaur.ActualDinosaurVersion
	// if dinosaurBeingUpgraded != dinosaur.DinosaurUpgrading {
	// 	logger.Logger.Infof("Dinosaur version for Dinosaur ID '%s' upgrade state changed from %t to %t", dinosaur.ID, prevDinosaurUpgrading, dinosaurBeingUpgraded)
	// 	dinosaur.DinosaurUpgrading = dinosaurBeingUpgraded
	// 	needsUpdate = true
	// }
...
func (d *dataPlaneDinosaurService) reassignDinosaurCluster(dinosaur *dbapi.DinosaurRequest) *serviceError.ServiceError {
	if dinosaur.Status == constants2.DinosaurRequestStatusProvisioning.String() {
		// If a Dinosaur cluster is rejected by the kas-fleetshard-operator, it should be assigned to another OSD cluster (via some scheduler service in the future).
...
func (d *dataPlaneDinosaurService) persistDinosaurRoutes(dinosaur *dbapi.DinosaurRequest, dinosaurStatus *dbapi.DataPlaneDinosaurStatus, cluster *api.Cluster) *serviceError.ServiceError {
	...
	if len(routesInRequest) == 0 {
		...
		oldMKIngressDNS := strings.Replace(clusterDNS, constants2.DefaultIngressDnsNamePrefix, constants2.ManagedDinosaurIngressDnsNamePrefix, 1)
		routes = dinosaur.GetDefaultRoutes(oldMKIngressDNS, d.dinosaurConfig.NumOfBrokers)
	} else {
		var routesErr error
