...
func (d *dataPlaneDinosaurService) setDinosaurClusterFailed(dinosaur *dbapi.DinosaurRequest, errMessage string) *serviceError.ServiceError {
	...
	logger.Logger.Errorf("Dinosaur status for Dinosaur ID '%s' in ClusterID '%s' reported as failed by KAS Fleet Shard Operator: '%s'", dinosaur.ID, dinosaur.ClusterID, errMessage)
