...
import (
	...
	manageddinosaur "github.com/bf2fc6cc711aee1a0c2a/kas-fleet-manager/pkg/api/manageddinosaurs.manageddinosaur.bf2.org/v1"
...
type DinosaurService interface {
	...
	VerifyAndUpdateDinosaur(ctx context.Context, dinosaurRequest *dbapi.DinosaurRequest) *errors.ServiceError
...
func (k *dinosaurService) RegisterDinosaurJob(dinosaurRequest *dbapi.DinosaurRequest) *errors.ServiceError {
	...
	if hasCapacity, err := k.HasAvailableCapacity(); err != nil {
		...
		logger.Logger.Warningf("Cluster capacity(%d) exhausted", k.dinosaurConfig.DinosaurCapacity.MaxCapacity)
		return errors.TooManyDinosaurInstancesReached("cluster capacity exhausted")
	}
	...
	dbConn := k.connectionFactory.New()
	dinosaurRequest.DesiredDinosaurVersion = k.dinosaurConfig.DefaultDinosaurVersion
...
func (k *dinosaurService) PrepareDinosaurRequest(dinosaurRequest *dbapi.DinosaurRequest) *errors.ServiceError {
	...
	if k.keycloakService.GetConfig().EnableAuthenticationOnDinosaur {
		...
		serviceAccountRequest := services.CompleteServiceAccountRequest{
			...
			ClientId:       fmt.Sprintf("%s-%s", CanaryServiceAccountPrefix, dinosaurRequest.ID),
...
func (k *dinosaurService) VerifyAndUpdateDinosaur(ctx context.Context, dinosaurRequest *dbapi.DinosaurRequest) *errors.ServiceError {
	...
	if auth.GetIsOrgAdminFromClaims(claims) && orgId == dinosaurRequest.OrganisationId {
		userValid, err := k.authService.CheckUserValid(dinosaurRequest.Owner, orgId)
		if err != nil {
		...
		if userValid {
			...
			return errors.NewWithCause(errors.ErrorBadRequest, err, "User %s does not belong in your organization", dinosaurRequest.Owner)
		}
	} else {
...
func (k *dinosaurService) ChangeDinosaurCNAMErecords(dinosaurRequest *dbapi.DinosaurRequest, action DinosaurRoutesAction) (*route53.ChangeResourceRecordSetsOutput, *errors.ServiceError) {
	...
	if routes == nil || err != nil {
		glog.Infof("failed to parse routes data for dinosaur %s, routes := %v", dinosaurRequest.ID, dinosaurRequest.Routes)
		if clusterDNS, err := k.clusterService.GetClusterDNS(dinosaurRequest.ClusterID); err != nil {
			return nil, err
		} else {
			routes = dinosaurRequest.GetDefaultRoutes(clusterDNS, k.dinosaurConfig.NumOfBrokers)
			glog.Infof("built default routes %v for dinosaur %s", dinosaurRequest.ID, routes)
		}
	}
...
func (k *dinosaurService) ListComponentVersions() ([]DinosaurComponentVersions, error) {
	...
	if err := dbConn.Model(&dbapi.DinosaurRequest{}).Select("id", "cluster_id", "desired_strimzi_version", "actual_strimzi_version", "strimzi_upgrading", "desired_dinosaur_version", "actual_dinosaur_version", "dinosaur_upgrading").Scan(&results).Error; err != nil {
