...
func Test_DataPlaneCluster_updateDataPlaneClusterNodes(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			inputFactory: func() *input {
				dinosaurConfig := sampleValidApplicationConfigForDataPlaneClusterTest(nil).DinosaurConfig
				testStatus := sampleValidBaseDataPlaneClusterStatusRequest()
				...
				testStatus.ResizeInfo.Delta.Connections = dinosaurConfig.DinosaurCapacity.TotalMaxConnections * 10
				testStatus.ResizeInfo.Delta.Partitions = dinosaurConfig.DinosaurCapacity.MaxPartitions * 10
				testStatus.Remaining.Connections = dinosaurConfig.DinosaurCapacity.TotalMaxConnections * 1000
				testStatus.Remaining.Partitions = dinosaurConfig.DinosaurCapacity.MaxPartitions * 1000
				apiCluster := &api.Cluster{
					ClusterID: testClusterID,
					MultiAZ:   true,
					Status:    api.ClusterReady,
		...
		},
		{
			...
			inputFactory: func() *input {
				...
				// We set remaining higher than a single dinosaur instance capacity to not
				// trigger scale-up and we set it less than delta values to not force a
				// scale-down
				testStatus.Remaining.Connections = dinosaurConfig.DinosaurCapacity.TotalMaxConnections * 2
				testStatus.Remaining.Partitions = dinosaurConfig.DinosaurCapacity.MaxPartitions * 2
				testStatus.ResizeInfo.Delta.Connections = dinosaurConfig.DinosaurCapacity.TotalMaxConnections * 10
				testStatus.ResizeInfo.Delta.Partitions = dinosaurConfig.DinosaurCapacity.MaxPartitions * 10
...
func Test_DataPlaneCluster_isFleetShardOperatorReady(t *testing.T) {
	tests := []struct {
		...
		{
			name: "When Fleet operator reports ready condition set to true the fleet shard operator is considered ready",
			clusterStatus: &dbapi.DataPlaneClusterStatus{
				Conditions: []dbapi.DataPlaneClusterStatusCondition{
					{
						Type:   "Ready",
		...
		{
			name: "When Fleet operator reports ready condition set to false the fleet shard operator is considered not ready",
			clusterStatus: &dbapi.DataPlaneClusterStatus{
				Conditions: []dbapi.DataPlaneClusterStatusCondition{
					{
						Type:   "Ready",
		...
		{
			name: "When Fleet operator reports doesn't report a Ready condition the fleet shard operator is considered not ready",
			clusterStatus: &dbapi.DataPlaneClusterStatus{
				Conditions: []dbapi.DataPlaneClusterStatusCondition{},
			},
			dataPlaneClusterServiceFactory: func() *dataPlaneClusterService {
		...
		{
			name: "When Fleet operator reports reports a Ready condition with an unknown value an error is returned",
			clusterStatus: &dbapi.DataPlaneClusterStatus{
				Conditions: []dbapi.DataPlaneClusterStatusCondition{
					{
						Type:   "Ready",
...
func Test_DataPlaneCluster_clusterCanProcessStatusReports(t *testing.T) {
	tests := []struct {
		...
		{
			name: "When cluster is waiting for Fleet Shard operator then status reports can be processed",
			apiCluster: &api.Cluster{
	...
	cases := []struct {
	...
	}{
		{
			...
			inputFactory: func() (*input, *api.ClusterStatus) {
				...
				dinosaurConfig := c.DinosaurConfig
				testStatus.NodeInfo.Current = 3
				testStatus.NodeInfo.Ceiling = 10000
				testStatus.NodeInfo.CurrentWorkLoadMinimum = 3
				testStatus.Remaining.Connections = dinosaurConfig.DinosaurCapacity.TotalMaxConnections + 1
				testStatus.Remaining.Partitions = dinosaurConfig.DinosaurCapacity.MaxPartitions + 1
