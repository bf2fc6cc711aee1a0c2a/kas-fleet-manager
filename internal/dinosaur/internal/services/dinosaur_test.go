	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				dinosaurRequest: buildDinosaurRequest(func(dinosaurRequest *dbapi.DinosaurRequest) {
					dinosaurRequest.ID = testID
					dinosaurRequest.CanaryServiceAccountClientID = "canary-id"
				}),
			},
			wantErr: true,
		},
		{
			...
			args: args{
				dinosaurRequest: buildDinosaurRequest(func(dinosaurRequest *dbapi.DinosaurRequest) {
					dinosaurRequest.CanaryServiceAccountClientID = ""
				}),
			},
			wantErr: false,
		},
	...
	tests := []struct {
		...
		},
		{
			...
			setupFn: func() {
				mocket.Catcher.Reset().
					NewMock().
					WithQuery(`SELECT count(1) FROM "dinosaur_requests" WHERE region = $1 AND "dinosaur_requests"."deleted_at" IS NULL`).
		...
		{
			name: "registering dinosaur job: region out of capacity",
			fields: fields{
				connectionFactory:      db.NewMockConnectionFactory(nil),
				clusterService:         nil,
				dinosaurConfig:         defaultDinosaurConf,
			...
			args: args{
				dinosaurRequest: buildDinosaurRequest(func(dinosaurRequest *dbapi.DinosaurRequest) {
					// we need to empty to ID otherwise an UPDATE will be performed instead of an insert
					dinosaurRequest.ID = ""
					dinosaurRequest.Region = "eu-west-1"
				}),
			},
			setupFn: func() {
				mocket.Catcher.Reset().NewMock().WithQuery("SELECT count").WithReply([]map[string]interface{}{{"count": "0"}})
...
func TestDinosaurService_CountByRegionAndInstanceType(t *testing.T) {
	type fields struct {
		connectionFactory *db.ConnectionFactory
	}
	tests := []struct {
		...
		fields    fields
		wantErr   bool
		want      []DinosaurRegionCount
		setupFunc func()
	}{
		{
			name:    "should return the counts of Dinosaurs per region and instance type",
			fields:  fields{connectionFactory: db.NewMockConnectionFactory(nil)},
			wantErr: false,
			setupFunc: func() {
				counters := []map[string]interface{}{
				...
				mocket.Catcher.Reset().
					NewMock().
					WithQuery(`SELECT region as Region, instance_type, cluster_id, count(1) as Count FROM "dinosaur_requests" WHERE "dinosaur_requests"."deleted_at" IS NULL GROUP BY region,instance_type,cluster_id`).
					WithReply(counters)
			},
			want: []DinosaurRegionCount{
				{
				...
				versions := []map[string]interface{}{
					{
						...
						"desired_dinosaur_ibp_version": "2.0",
						"actual_dinosaur_ibp_version":  "2.0",
						"dinosaur_ibp_upgrading":       false,
					},
					{
						"id":                           "2",
						"cluster_id":                   "cluster2",
						...
						"actual_dinosaur_version":      "2.0.0",
						"dinosaur_upgrading":           false,
						"desired_dinosaur_ibp_version": "2.2",
						"actual_dinosaur_ibp_version":  "2.1",
						"dinosaur_ibp_upgrading":       true,
					},
				}
				mocket.Catcher.Reset().
					NewMock().
					WithQuery(`SELECT "id","cluster_id","desired_strimzi_version","actual_strimzi_version","strimzi_upgrading","desired_dinosaur_version","actual_dinosaur_version","dinosaur_upgrading","desired_dinosaur_ibp_version","actual_dinosaur_ibp_version","dinosaur_ibp_upgrading"`).
					WithReply(versions)
			},
			want: []DinosaurComponentVersions{{
				ID:                        "1",
				...
				DesiredDinosaurIBPVersion: "2.0",
				ActualDinosaurIBPVersion:  "2.0",
				...
				ActualDinosaurVersion:     "2.0.0",
				DinosaurUpgrading:         false,
				DesiredDinosaurIBPVersion: "2.2",
				ActualDinosaurIBPVersion:  "2.1",
