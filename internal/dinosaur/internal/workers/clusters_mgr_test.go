...
var (
	...
	testProvider                  = "aws"
	strimziAddonID                = "managed-dinosaur-test"
...
func TestClusterManager_reconcileAddonOperator(t *testing.T) {
	type fields struct {
		agentOperator  services.FleetshardOperatorAddon
		clusterService services.ClusterService
	}
	tests := []struct {
		name    string
		...
		{
			name: "successful strimzi cluster, logging operator and fleetshard operator installation",
			fields: fields{
				agentOperator: &services.FleetshardOperatorAddonMock{
				...
				clusterService: &services.ClusterServiceMock{
					...
					UpdateStatusFunc: func(cluster api.Cluster, status api.ClusterStatus) error {
						if status != api.ClusterWaitingForFleetShardOperator {
							t.Errorf("expect status to be %s but got %s", api.ClusterWaitingForFleetShardOperator.String(), status)
						}
						return nil
					},
				},
		...
		{
			name: "return an error if fleetshard operator installation fails",
			fields: fields{
				clusterService: &services.ClusterServiceMock{
					InstallStrimziFunc: func(cluster *api.Cluster) (bool, *apiErrors.ServiceError) {
						return false, nil
				...
				agentOperator: &services.FleetshardOperatorAddonMock{
					ProvisionFunc: func(cluster api.Cluster) (bool, *apiErrors.ServiceError) {
						return false, apiErrors.GeneralError("failed to provision fleetshard operator")
					},
				},
			},
			wantErr: true,
		...
		t.Run(tt.name, func(t *testing.T) {
			c := &ClusterManager{
				ClusterManagerOptions: ClusterManagerOptions{
					...
					ClusterService:             tt.fields.clusterService,
					FleetshardOperatorAddon: tt.fields.agentOperator,
				},
			}
	...
	tests := []struct {
...

		{
			...
			fields: fields{
				...
				osdIDPKeycloakService: &coreServices.KeycloakServiceMock{
					DeRegisterClientInSSOFunc: func(dinosaurNamespace string) *apiErrors.ServiceError {
		...
		{
			name: "recieves an error when remove fleetshard-operator service account fails",
			fields: fields{
				clusterService: &services.ClusterServiceMock{
					UpdateStatusFunc: func(cluster api.Cluster, status api.ClusterStatus) error {
						return nil
				...
				fleetshardOperatorAddon: &services.FleetshardOperatorAddonMock{
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			c := &ClusterManager{
				ClusterManagerOptions: ClusterManagerOptions{
					ClusterService:             tt.fields.clusterService,
					OsdIdpKeycloakService:      tt.fields.osdIDPKeycloakService,
					FleetshardOperatorAddon: tt.fields.fleetshardOperatorAddon,
				},
			}
	...
	if clusterCreateConfig.ImagePullDockerConfigContent != "" {
		resources = append(resources, &k8sCoreV1.Secret{
			...
			&k8sCoreV1.Secret{
				...
				ObjectMeta: metav1.ObjectMeta{
					Name:      imagePullSecretName,
					Namespace: fleetshardAddonNamespace,
				},
				Type: k8sCoreV1.SecretTypeDockercfg,
				Data: map[string][]byte{
					k8sCoreV1.DockerConfigKey: []byte(clusterCreateConfig.ImagePullDockerConfigContent),
...
func TestClusterManager_reconcileClusterWithManualConfig(t *testing.T) {
	...
	testOsdConfig.ClusterConfig = config.NewClusterConfig(config.ClusterList{config.ManualCluster{Schedulable: true, DinosaurInstanceLimit: 2}})
	tests := []struct {
		name    string
		fields  fields
		wantErr bool
	}{
		{
			...
			fields: fields{
				clusterService: &services.ClusterServiceMock{
					...
					FindDinosaurInstanceCountFunc: func(clusterIDs []string) ([]services.ResDinosaurInstanceCount, *apiErrors.ServiceError) {
						return []services.ResDinosaurInstanceCount{
							{
								Clusterid: "test02",
								Count:     1,
							},
		...
		},
		{
			...
			fields: fields{
				clusterService: &services.ClusterServiceMock{
					...
					FindDinosaurInstanceCountFunc: func(clusterIDs []string) ([]services.ResDinosaurInstanceCount, *apiErrors.ServiceError) {
						return []services.ResDinosaurInstanceCount{}, nil
					},
				},
				DataplaneClusterConfig: testOsdConfig,
			},
...
func TestClusterManager_reconcileClusterInstanceType(t *testing.T) {
	...
	testOsdConfig.ClusterConfig = config.NewClusterConfig(config.ClusterList{config.ManualCluster{Schedulable: true, DinosaurInstanceLimit: 2, ClusterId: clusterId, SupportedInstanceType: supportedInstanceType}})
	tests := []struct {
		name    string
		fields  fields
		wantErr bool
