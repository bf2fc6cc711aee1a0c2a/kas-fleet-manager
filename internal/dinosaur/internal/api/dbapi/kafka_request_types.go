...
func (k *DinosaurRequest) GetDefaultRoutes(clusterDNS string, numberOfBrokers int) []DataPlaneDinosaurRoute {
	clusterIngress := fmt.Sprintf("elb.%s", clusterDNS)

	routes := []DataPlaneDinosaurRoute{
		{
