...
func (h *dataPlaneClusterHandler) validateStrimzi(request *private.DataPlaneClusterUpdateStatusRequest) *errors.ServiceError {
	for idx, strimziElem := range request.Strimzi {
		...
		if len(strimziElem.DinosaurVersions) == 0 {
			return errors.FieldValidationError(fmt.Sprintf(".status.strimzi[%d].dinosaurVersions cannot be empty", idx))
		}

		if len(strimziElem.DinosaurIbpVersions) == 0 {
			return errors.FieldValidationError(fmt.Sprintf(".status.strimzi[%d].dinosaurIbpVersions cannot be empty", idx))
		}
	}
