...
func resetCanaryServiceAccountForAffectedInstances() *gormigrate.Migration {
	return &gormigrate.Migration{
		...
		Migrate: func(tx *gorm.DB) error {
			...
			query := fmt.Sprintf("update dinosaur_requests set canary_service_account_client_id='', canary_service_account_client_secret = '' where canary_service_account_client_id in %s;", affectedInstancesServiceAccountClientIds)
