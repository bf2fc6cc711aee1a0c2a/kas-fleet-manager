...
func TestClusterManager_SuccessfulReconcile(t *testing.T) {
	...
	// start servers
	h, _, teardown := test.NewDinosaurHelperWithHooks(t, ocmServer, func(c *ocm.OCMConfig, d *config.DataplaneClusterConfig) {
		c.ClusterLoggingOperatorAddonID = ocm.ClusterLoggingOperatorAddonID
		d.ClusterConfig = config.NewClusterConfig([]config.ManualCluster{test.NewMockDataplaneCluster(mockDinosaurClusterName, 1)})
	})
	defer teardown()
