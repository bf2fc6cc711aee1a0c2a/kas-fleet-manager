...
func TestDataPlaneCluster_ClusterStatusTransitionsToReadySuccessfully(t *testing.T) {
	...
	testDataPlaneclusterID, getClusterErr := common.GetOSDClusterIDAndWaitForStatus(h, t, api.ClusterWaitingForFleetShardOperator)
	...
	ctx := fleetshardsync.NewAuthenticatedContextForDataPlaneCluster(h, testDataPlaneclusterID)
	privateAPIClient := test.NewPrivateAPIClient(h)

	commonDinosaurVersions := []string{"2.8.0", "1.3.6", "2.7.0"}
	commonDinosaurIBPVersions := []string{"2.8", "1.3", "2.7"}
	clusterStatusUpdateRequest := fleetshardsync.SampleDataPlaneclusterStatusRequestWithAvailableCapacity()
	clusterStatusUpdateRequest.Strimzi = []private.DataPlaneClusterUpdateStatusRequestStrimzi{
		private.DataPlaneClusterUpdateStatusRequestStrimzi{
			...
			Ready:            true,
			DinosaurVersions:    commonDinosaurVersions,
			DinosaurIbpVersions: commonDinosaurIBPVersions,
		},
		private.DataPlaneClusterUpdateStatusRequestStrimzi{
	...
	expectedCommonDinosaurVersions := []api.DinosaurVersion{
		api.DinosaurVersion{Version: "1.3.6"},
		api.DinosaurVersion{Version: "2.7.0"},
		api.DinosaurVersion{Version: "2.8.0"},
	}
	expectedCommonDinosaurIBPVersions := []api.DinosaurIBPVersion{
		api.DinosaurIBPVersion{Version: "1.3"},
		api.DinosaurIBPVersion{Version: "2.7"},
		api.DinosaurIBPVersion{Version: "2.8"},
	}
	expectedAvailableStrimziVersions := []api.StrimziVersion{
		{Version: "strimzi-cluster-operator.v.3.0.0-0", Ready: true, DinosaurVersions: expectedCommonDinosaurVersions, DinosaurIBPVersions: expectedCommonDinosaurIBPVersions},
		{Version: "strimzi-cluster-operator.v.5.8.0-0", Ready: true, DinosaurVersions: expectedCommonDinosaurVersions, DinosaurIBPVersions: expectedCommonDinosaurIBPVersions},
		{Version: "strimzi-cluster-operator.v.5.12.0-0", Ready: true, DinosaurVersions: expectedCommonDinosaurVersions, DinosaurIBPVersions: expectedCommonDinosaurIBPVersions},
	}
...
func TestDataPlaneCluster_GetManagedDinosaurAgentCRSuccess(t *testing.T) {
	ocmServer := mocks.NewMockConfigurableServerBuilder().Build()
	defer ocmServer.Close()
...
func TestDataPlaneCluster_ClusterStatusTransitionsToFullWhenNoMoreDinosaurCapacity(t *testing.T) {
	ocmServerBuilder := mocks.NewMockConfigurableServerBuilder()
	mockedGetClusterResponse, err := mockedClusterWithMetricsInfo(mocks.MockClusterComputeNodes)
	if err != nil {
		t.Fatalf(err.Error())
	...
	// Create dummy dinosaur and assign it to dummy cluster to make it not empty.
	// This is done so it is not scaled down by the dynamic scaling
	// functionality
	dummyDinosaur := dbapi.DinosaurRequest{
		ClusterID:     dummyClusterID,
		MultiAZ:       false,
		Region:        cluster.Region,
		CloudProvider: cluster.CloudProvider,
	...
	if err := db.Save(&dummyDinosaur).Error; err != nil {
		t.Error("failed to create a dummy dinosaur request")
		return
	}

	// Create dummy dinosaur and assign it to test data plane cluster to make it not
	// empty. This is done so it is not scaled down by the dynamic scaling
	// functionality
	dummyDinosaur.ID = api.NewID()
	dummyDinosaur.ClusterID = testDataPlaneclusterID
	if err := db.Save(&dummyDinosaur).Error; err != nil {
		t.Error("failed to create a dummy dinosaur request")
		return
	}
...
func TestDataPlaneCluster_ClusterStatusTransitionsToWaitingForFleetOperatorWhenOperatorIsNotReady(t *testing.T) {
	ocmServer := mocks.NewMockConfigurableServerBuilder().Build()
	defer ocmServer.Close()
	...
	Expect(err).ToNot(HaveOccurred())
	Expect(cluster).ToNot(BeNil())
	Expect(cluster.Status).To(Equal(api.ClusterWaitingForFleetShardOperator))
}
...
func TestDataPlaneCluster_TestOSDClusterScaleUp(t *testing.T) {
	...
	Expect(err).ToNot(HaveOccurred())
	err = db.Save(&dbapi.DinosaurRequest{ClusterID: testDataPlaneclusterID, Status: string(constants2.DinosaurRequestStatusReady)}).Error
	Expect(err).ToNot(HaveOccurred())
...
func TestDataPlaneCluster_WhenReportedStrimziVersionsAreDifferentClusterStrimziVersionsIsUpdated(t *testing.T) {
	...
	commonDinosaurVersions := []string{"2.8.0", "2.7.0"}
	commonDinosaurIBPVersions := []string{"2.8", "2.7"}
	...
	expectedAvailableStrimziVersions := []api.StrimziVersion{
		{
			Version:          "strimzi-cluster-operator.v.3.0.0-0",
			Ready:            true,
			DinosaurVersions:    expectedCommonDinosaurVersions,
			DinosaurIBPVersions: expectedCommonDinosaurIBPVersions,
		},
		{
			Version:          "strimzi-cluster-operator.v.5.0.0-0",
			Ready:            false,
...
func DinosaurConfig(h *coreTest.Helper) (c *config.DinosaurConfig) {
	h.Env.MustResolve(&c)
	return
}
...
func newAuthContextWithNotAllowedClusterIDForDataPlaneCluster(h *coreTest.Helper) context.Context {
	...
	claims := jwt.MapClaims{
		...
		"fleetshard-operator-cluster-id": "differentcluster",
	}
	token := h.CreateJWTStringWithClaim(account, claims)
	ctx := context.WithValue(context.Background(), private.ContextAccessToken, token)
