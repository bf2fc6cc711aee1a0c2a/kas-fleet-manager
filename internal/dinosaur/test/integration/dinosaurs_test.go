...
func TestDinosaurCreate_Success(t *testing.T) {
	...
	Expect(foundDinosaur.Version).To(Equal(kasfleetshardsync.GetDefaultReportedDinosaurVersion()))
	...
	Expect(dinosaur.Version).To(Equal(kasfleetshardsync.GetDefaultReportedDinosaurVersion()))
...
func TestDinosaur_Update(t *testing.T) {
	...
	emptyOwnerDinosaurUpdateReq := public.DinosaurUpdateRequest{}
	sameOwnerDinosaurUpdateReq := public.DinosaurUpdateRequest{Owner: owner}
	newOwnerDinosaurUpdateReq := public.DinosaurUpdateRequest{Owner: "test_dinosaur_service"}
...
func TestDinosaurCreate_OverrideDesiredStrimziVersion(t *testing.T) {
	// create a mock ocm api server, keep all endpoints as defaults
	// see the mocks package for more information on the configurable mock server
	ocmServer := mocks.NewMockConfigurableServerBuilder().Build()
	defer ocmServer.Close()
	...
	// setup the test environment, if OCM_ENV=integration then the ocmServer provided will be used instead of actual
	// ocm
	h, client, teardown := test.NewDinosaurHelperWithHooks(t, ocmServer, func(config *config.DataplaneClusterConfig) {
	...
	dinosaur, _, err := common.WaitForDinosaurCreateToBeAccepted(ctx, test.TestServices.DBFactory, client, k)
	// dinosaur successfully registered with database
	Expect(err).NotTo(HaveOccurred(), "Error posting object:  %v", err)
	...
	Expect(dinosaurRequest.DesiredStrimziVersion).To(Equal(desiredStrimziVersion))
	// delete test dinosaur to free up resources on an OSD cluster
	deleteTestDinosaur(t, h, ctx, client, foundDinosaur.Id)
}
...
func TestDinosaurCreate_TooManyDinosaurs(t *testing.T) {
	...
	// setup the test environment, if OCM_ENV=integration then the ocmServer provided will be used instead of actual
	// ocm
	h, client, tearDown := test.NewDinosaurHelperWithHooks(t, ocmServer, func(c *config.DinosaurConfig) {
		c.DinosaurCapacity.MaxCapacity = 2
	})
	defer tearDown()
...
func TestDinosaurQuotaManagementList_MaxAllowedInstances(t *testing.T) {
	...
	h, client, teardown := test.NewDinosaurHelperWithHooks(t, ocmServer, func(acl *quota_management.QuotaManagementListConfig) {
...
func TestDinosaurDelete_DeleteDuringCreation(t *testing.T) {
	...
	h, client, teardown := test.NewDinosaurHelperWithHooks(t, ocmServer, func(ocmConfig *ocm.OCMConfig) {
	...
	mockKasFleetshardSyncBuilder.SetUpdateDinosaurStatusFunc(func(helper *coreTest.Helper, privateClient *private.APIClient) error {
