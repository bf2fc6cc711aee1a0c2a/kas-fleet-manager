...
func ConvertConnectorInstance(from public.ConnectorInstance) (*dbapi.Connector, *errors.ServiceError) {
...
func PresentConnectorInstance(from *dbapi.Connector) (public.ConnectorInstance, *errors.ServiceError) {
	spec := map[string]interface{}{}
	err := json.Unmarshal([]byte(from.ConnectorSpec), &spec)
	if err != nil {
		return public.ConnectorInstance{}, errors.BadRequest("invalid connector spec: %v", err)
	}

	reference := PresentReference(from.ID, from)
	return public.ConnectorInstance{
		Id:   reference.Id,
		Kind: reference.Kind,
		Href: reference.Href,
