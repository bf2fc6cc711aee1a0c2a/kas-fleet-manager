...
func (m *ProcessorManager) Reconcile() []error {
	...
	// Delete Processor once it has transitioned to Deleted by FleetShard
	m.doReconcile(&errs, "deleting", m.reconcileDeleting,
		"desired_state = ? AND phase = ?", dbapi.ProcessorDeleted, dbapi.ProcessorStatusPhaseDeleting)
...
func (m *ProcessorManager) reconcileDeleting(ctx context.Context, processor *dbapi.Processor) error {
