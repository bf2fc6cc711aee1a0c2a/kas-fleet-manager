...
func addConnectorNamespaceDeployment(migrationId string) *gormigrate.Migration {
	...
	return db.CreateMigrationFromActions(migrationId,
		// delete old connector deployments, connectors and clusters
		db.ExecAction(`DELETE FROM connector_deployment_statuses`, ``),
		db.ExecAction(`DELETE FROM connector_deployments`, ``),
		db.ExecAction(`DELETE FROM connector_statuses`, ``),
		db.ExecAction(`DELETE FROM connectors`, ``),
		db.ExecAction(`DELETE FROM connector_clusters`, ``),
