	...
	tests := []struct {
		...
		},
		{
			...
			fields: fields{
				...
				certManagementClient: &certMagicClientWrapperMock{
					RevokeCertFunc: nil, // it should never be called
				},
			},
			args: args{
				domain: "some-domain",
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{
				...
				certManagementClient: &certMagicClientWrapperMock{
					ManageSyncFunc: nil, // it should never be called
				},
			},
			args: args{
				domain: "some-domain",
				...
				certManagementClient: &certMagicClientWrapperMock{
					ManageSyncFunc: func(ctx context.Context, domainNames []string) error {
						if !strings.HasPrefix(domainNames[0], "*") {
							panic("wildcard certificate has to be asked as per ADR-89: https://github.com/bf2fc6cc711aee1a0c2a/architecture/blob/main/_adr/89/index.adoc")
						}
						return nil
					},
