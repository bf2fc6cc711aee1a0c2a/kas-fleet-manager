...
func TestPresentKafkaRequest(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			Expect(PresentKafkaRequest(tt.args.dbKafkaRequest, "", tt.args.kafkaConfig)).To(Equal(tt.want))
		})
	}
}
...
func TestCapacityLimitReports(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			kafkaRequest, err := PresentKafkaRequest(&test.request, test.config.BrowserUrl, &test.config)
			if !test.errExpected {
				if !test.negative {
