...
func GetSupportedKafkaSizes(supportedKafkaSizes []api.SupportedKafkaSize) []public.SupportedKafkaSize {
	...
	for _, c := range supportedKafkaSizes {
		sizes = append(sizes, public.SupportedKafkaSize{Id: c.Id, IngressThroughputPerSec: c.IngressThroughputPerSec, EgressThroughputPerSec: c.EgressThroughputPerSec, TotalMaxConnections: c.TotalMaxConnections, MaxDataRetentionSize: c.MaxDataRetentionSize, MaxPartitions: c.MaxPartitions, MaxDataRetentionPeriod: c.MaxDataRetentionPeriod, MaxConnectionAttemptsPerSec: c.MaxConnectionAttemptsPerSec, QuotaConsumed: c.QuotaConsumed, QuotaType: c.QuotaType, CapacityConsumed: c.CapacityConsumed})
	}
	return sizes
}
