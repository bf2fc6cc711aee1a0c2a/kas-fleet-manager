...
aws/setup:
	...
	@echo -n "$(AWS_SECRET_MANAGER_ACCESS_KEY)" > secrets/vault/aws_access_key_id
	@echo -n "$(AWS_SECRET_MANAGER_SECRET_ACCESS_KEY)" > secrets/vault/aws_secret_access_key
.PHONY: aws/setup
...
deploy/secrets:
	...
	@$(OC) process -f ./templates/secrets-template.yml \
		...
		-p AWS_SECRET_MANAGER_ACCESS_KEY="$(shell ([ -s './secrets/vault/aws_access_key_id' ] && [ -z '${AWS_SECRET_MANAGER_ACCESS_KEY}' ]) && cat ./secrets/vault/aws_access_key_id || echo '${AWS_SECRET_MANAGER_ACCESS_KEY}')" \
		-p AWS_SECRET_MANAGER_SECRET_ACCESS_KEY="$(shell ([ -s './secrets/vault/aws_secret_access_key' ] && [ -z '${AWS_SECRET_MANAGER_SECRET_ACCESS_KEY}' ]) && cat ./secrets/vault/aws_secret_access_key || echo '${AWS_SECRET_MANAGER_SECRET_ACCESS_KEY}')" \
...
deploy/service: OBSERVABILITY_OPERATOR_INDEX_IMAGE ?= "quay.io/rhoas/observability-operator-index:v4.0.2"
deploy/service: OBSERVABILITY_OPERATOR_STARTING_CSV ?= "observability-operator.v4.0.2"
...
deploy/service: KAFKA_TLS_CERTIFICATE_MANAGEMENT_STORAGE_TYPE ?= "vault"
