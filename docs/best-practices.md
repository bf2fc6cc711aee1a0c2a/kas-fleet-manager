...
This document covers the best practice we follow when developing the KAS Fleet Manager.
...
```
Instead, make use of `gomega` and use `Equal`:
```go
...
For more tips, look [here](https://github.com/golang/go/wiki/CodeReviewComments)
