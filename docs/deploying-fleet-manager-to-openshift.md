...
2. [Build and Push Fleet Manager Image](#build-and-push-kas-fleet-manager-image-to-the-openshift-internal-registry)
...
6. [Deploy Fleet Manager](#deploy-kas-fleet-manager)
7. [Access the Service](#access-the-service)
8. [Removing Fleet Manager from OpenShift](#removing-kas-fleet-manager-from-openshift)
...
- `ENABLE_KAFKA_EXTERNAL_CERTIFICATE`: Enable Dinosaur TLS Certificate. Defaults to `false`.
- `ENABLE_KAFKA_LIFE_SPAN`: Enables Dinosaur expiration. Defaults to `false`.
- `KAFKA_LIFE_SPAN`: Dinosaur expiration lifetime in hours. Defaults to `48`.
...
- `KAS_FLEETSHARD_OLM_INDEX_IMAGE`: Fleetshard operator OLM index image. Defaults to `quay.io/osd-addons/kas-fleetshard-operator:production-82b42db`.
