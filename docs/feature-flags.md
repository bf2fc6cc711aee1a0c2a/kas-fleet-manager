...
- **strimzi-operator-version** The version of the Strimzi operator to install. If set, DesiredStrimziVersion in kafka_request will be updated with this value. Otherwise value reported from kas-fleetshard operator will be used
...
- **enable-jwt**: Enables JSON Web Token (JWT) validation.
    - One or more of the following configuration must be specified:
        ...
        - `token-issuer-url`: The URL to the token issuer. Used to validate if the token was issued from the given issuer only. (default: `https://sso.redhat.com/auth/realms/redhat-external`) 
