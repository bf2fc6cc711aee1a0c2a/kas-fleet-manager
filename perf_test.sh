...
check_params () {
  if [[ -z "${OCM_OFFLINE_TOKEN}" || \
      ...
      -z "${PERF_TEST_SINGLE_WORKER_DINOSAUR_CREATE}" || \
      -z "${PERF_TEST_CLEANUP}" || \
      -z "${PERF_TEST_HIT_ENDPOINTS_HOLD_OFF}" || \
      -z "${PERF_TEST_DINOSAUR_POST_WAIT_TIME}" || \
      -z "${PERF_TEST_GET_ONLY}" || \
      -z "${PERF_TEST_DINOSAURS_PER_WORKER}" || \
      -z "${PERF_TEST_PREPOPULATE_DB_DINOSAUR_PER_WORKER}" || \
