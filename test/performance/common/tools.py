...
dinosaur_config_write_url = 'http://api:8099/write_dinosaur_config'
...
# get details of service account associated with dinosaur
def get_svc_account_for_dinosaur(collection, dinosaur_id):
  for item in collection:
    if 'dinosaur_id' in item and item['dinosaur_id'] == dinosaur_id:
      return item
  return []

# send dinosaur bootstrap URL and svc acc credentials to the helper API
# which will persist it to a text file
def persist_dinosaur_config(bootstrapURL, svc_acc_json):
  config = {
             'bootstrapURL': bootstrapURL,
             'username': svc_acc_json['client_id'],
             'password': svc_acc_json['client_secret'],
           }
  persist_to_api_helper(dinosaur_config_write_url, config)
